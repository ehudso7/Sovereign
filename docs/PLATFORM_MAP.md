...
| Environment  | Typical Contents                                   | Protection Rules       |
| ------------ | -------------------------------------------------- | ---------------------- |
| `staging`    | Optional environment-scoped staging credentials    | None (auto-deploy OK)  |
| `production` | Optional environment-scoped production credentials | **Required reviewers** |

GitHub Actions in this repo support either repository-level secrets with explicit suffixes such as `RAILWAY_TOKEN_STAGING` and `RAILWAY_TOKEN_PRODUCTION`, or environment-scoped secrets when you prefer to keep credentials attached to GitHub Environments.
...
| ------------------------------------ | ------------------------------------------- | ------------------------------- |
| **Temporal Cloud** (preferred)       | `<namespace>.<account_id>.tmprl.cloud:7233` | API key or mTLS certificates    |
